...
# add fusion module in encoder and decoder: debugging!!!!
from .deformable_transformer_deep import build_deforamble_transformer, FeatureResizer, VisionLanguageFusionModule
from .fusion_module import MultiVisionLanguageFusion
...
class ClipMatcher(SetCriterion):
    ...
    def loss_refers(self,  outputs, gt_instances: List[Instances], indices: List[tuple], num_boxes):
        """
            args:
                outputs: model outputs -> logits, boxes, refers
                gt_instances: GT -> boxes, labels, obj_ids, area, is_ref
                indices: indices of matched gt boxes and track query boxes -> (src_idx, tgt_idx)
        """
        filtered_idx = []
        for src_per_img, tgt_per_img in indices:
            keep = tgt_per_img != -1 # 过滤无效的匹配
            filtered_idx.append((src_per_img[keep], tgt_per_img[keep]))
        indices = filtered_idx
        idx = self._get_src_permutation_idx(indices)
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        # if 'aux_outputs' in outputs:
        #     for i, aux_outputs in enumerate(outputs['aux_outputs']):
        #         # step1. Extract object query and track query for this layer
        #         pred_logits_aux = aux_outputs['pred_logits'][0].unsqueeze(0)  # shape: (1, num_queries, num_classes)
        #         pred_boxes_aux = aux_outputs['pred_boxes'][0].unsqueeze(0)
        #         pred_refers_aux = aux_outputs.get('pred_refers', torch.zeros_like(pred_logits_aux[..., :1]))[0].unsqueeze(0)  # 兼容性处理

        #         # step2. Match both object query and track query to all GTs
        #         # First, perform object query matching (object query与所有的GT匹配)
        #         object_query_outputs = {
        #             'pred_logits': pred_logits_aux[:, :self.num_object_queries, :],  # using the same logits for simplicity
        #             'pred_boxes': pred_boxes_aux[:, :self.num_object_queries, :],
        #             'pred_refers': pred_refers_aux[:, :self.num_object_queries, :],
        #         }
                
        #         # object_query_matched_indices = match_for_single_decoder_layer(object_query_outputs, self.matcher)
        #         object_query_matched_indices = self.matcher(object_query_outputs, [gt_instances_i])
        #         src_idx = object_query_matched_indices[0][0]
        #         tgt_idx = object_query_matched_indices[0][1]
        #         object_query_matched_indices = torch.stack([src_idx, tgt_idx], dim=1).to(pred_logits_aux.device)

        #         # Then, perform track query matching (track query与所有的GT匹配)
        #         if pred_logits_aux[:, self.num_object_queries:, :].shape[1] != 0:
        #             track_query_outputs = {
        #                 'pred_logits': pred_logits_aux[:, self.num_object_queries:, :],  # using the same logits for simplicity
        #                 'pred_boxes': pred_boxes_aux[:, self.num_object_queries:, :],
        #                 'pred_refers': pred_refers_aux[:, self.num_object_queries:, :],
        #             }
        #             track_query_matched_indices = match_for_single_decoder_layer(track_query_outputs, self.matcher)
        #             # track_query_matched_indices = self.matcher(track_query_outputs, [gt_instances_i])
        #             # src_idx = track_query_matched_indices[0][0] + 300
        #             # tgt_idx = track_query_matched_indices[0][1]
        #             # track_query_matched_indices = torch.stack([src_idx, tgt_idx], dim=1).to(pred_logits_aux.device)

        #             combined_matched_indices = torch.cat([object_query_matched_indices, track_query_matched_indices], dim=0)
        #         else:
        #             combined_matched_indices = object_query_matched_indices

        #         # step3. Calculate losses based on combined matched indices
        #         for loss in losses:
        #             if loss == 'masks':
        #                 continue  # Skip masks for now, assuming it's too costly to compute.
        #             l_dict = self.get_loss(
        #                 loss,
        #                 aux_outputs,
        #                 gt_instances=[gt_instances_i],
        #                 indices=[(combined_matched_indices[:, 0], combined_matched_indices[:, 1])],
        #                 num_boxes=1,
        #             )
        #             self.losses_dict.update(
...
class TransRMOTV3(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        # for history memory
        self.hist_length = history_memory_length # for MQU history memory length
        ...
        # loaded local model
        self.tokenizer = RobertaTokenizerFast.from_pretrained('/home/sq_2023/rmot/transrmot/weights/roberta-base',
                                                              local_files_only=True)
        self.text_encoder = RobertaModel.from_pretrained('/home/sq_2023/rmot/transrmot/weights/roberta-base',
                                                         local_files_only=True)
        ...
        # new
        self.fusion_module = MultiVisionLanguageFusion(d_model=hidden_dim, nhead=8)
    ...
    def _generate_empty_tracks(self):
        ...
        # for history memory
        track_instances.hist_memory = torch.zeros((len(track_instances), self.hist_length, dim // 2), dtype=torch.float,device=device) # (num_queries, length, dim)
        track_instances.hist_memory_padding_mask = torch.ones((len(track_instances), self.hist_length), dtype=torch.bool, device=device)
        track_instances.last_output = torch.zeros((len(track_instances), dim // 2), dtype=torch.float, device=device)
    ...
    def forward_textv2(self, text_queries, device):
        ...
        text_sentence_features = text_features[:, 0, :]
        text_word_features = text_features[:, 1:-1, :]
        text_word_mask = text_pad_mask[:, 1:-1]
            
        return text_word_features, text_word_mask, text_sentence_features
    ...
    def _forward_single_image(self, samples, track_instances: Instances, sentences):
        ...
        text_word_features, text_word_mask, text_sentence_features = self.forward_textv2(sentences, src.device)
        ...
        text_dict = {
            ...
            "text_word_mask": text_word_mask,  # bs, 195
            "text_pos": text_pos  # 195, bs, d_model
        }
        ...
        out['hs'] = hs[-1]
        out['text_embeddings'] = text_dict['text_word_features']
        return out
        ...
        for frame_index, frame in enumerate(frames):
            ...
            if self.use_checkpoint and frame_index < len(frames) - 1:
                ...
                frame_res = {
                    ...
                    'ref_pts': tmp[3],
                    'hs': tmp[4],
                    'text_embeddings': tmp[5],
                    'aux_outputs': [{
                        'pred_logits': tmp[6+i],
                        'pred_boxes': tmp[6+5+i],
                        'pred_refers': tmp[6+5+5+i]
                    } for i in range(5)],
                }
            else:
                frame = nested_tensor_from_tensor_list([frame])
