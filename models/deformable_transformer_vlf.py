...
class DeformableTransformer(nn.Module):
    def __init__(self, 
        ...
        # fusion module from Grounding DINO
        self.fusion_module = BiAttentionBlock(v_dim=v_dim,l_dim=l_dim,
                                              embed_dim=embed_dim,
                                              num_heads=nhead,
                                              dropout=dropout,)
        self.feature_resizer = FeatureResizer(v_dim, d_model, dropout, do_ln=True)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, text_dict=None, 
        ...
        # fusion image and text features
        bs, size, c = memory.shape
        # word level fusion
        memory, text_memory = self.fusion_module(memory, text_dict['text_word_features'])
        text_dict['text_word_features'] = text_memory
        sentence_embeds = text_dict['text_sentences_features'].unsqueeze(1).expand(-1, size, -1)
        if sentence_fusion_mode == 'add':
            # sentence level fusion（直接相加）
            memory = memory + sentence_embeds
            memory = self.feature_resizer(memory)

        elif sentence_fusion_mode == 'contact':
            # sentence level fusion（contact后再通过 MLP 映射）
            memory = torch.cat([memory, sentence_embeds], dim=-1)
