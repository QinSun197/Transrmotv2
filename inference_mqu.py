...
from main_debug import get_args_parser
...
def draw_bboxes(ori_img, bbox, identities=None, offset=(0, 0), cvt_color=False):
    ...
    for i, box in enumerate(bbox):
        ...
        y2 += offset[1]
        if len(box) > 5:
            score = '{:.2f}'.format(box[5]) # pred_refers
            # score = '{:.2f}'.format(box[4]) #scores of bbox            
        elif len(box) > 4:
        ...
        # label = '{}:{:.2f}'.format(label, score) if score is not None else label
...
def draw_points(img: np.ndarray, points: np.ndarray, color=(255, 255, 255)) -> np.ndarray:
    ...
    for i, (x, y) in enumerate(points):
        if i >= 300:
            ...
            color = (173, 216, 230)
        cv2.circle(img, (int(x), int(y)), 3, color=color, thickness=-1)
    return img
...
class TransRMOT(object):
    ...
    def update(self, dt_instances: Instances):
        ...
        for i in range(len(dt_instances)):
            ...
            bbox = np.concatenate([dt_instances.pred_boxes[i], dt_instances.scores[i:i + 1]], axis=-1)
        ...
        for i in range(len(dt_instances)):
            ...
            if label == 0:
                ...
                box_with_score = np.concatenate([dt_instances.pred_boxes[i], dt_instances.scores[i:i + 1]], axis=-1)
                ret.append(
...
class Detector(object):
    ...
    def filter_dt_by_ref_scores(dt_instances: Instances, ref_threshold: float) -> Instances:
        keep = dt_instances.pred_refers > ref_threshold
        return dt_instances[keep]
    ...
    def visualize_img_with_bbox(img_path, img, dt_instances: Instances, ref_pts=None, gt_boxes=None):
        if dt_instances.has('scores'):
            img_show = draw_bboxes(img,
                                   np.concatenate([dt_instances.pred_boxes, dt_instances.scores.reshape(-1, 1), dt_instances.pred_refers.reshape(-1, 1)], axis=-1),
                                   dt_instances.obj_idxes)
        else:
            img_show = draw_bboxes(img, dt_instances.pred_boxes, dt_instances.obj_idxes)
        if ref_pts is not None and len(ref_pts) !=0 :
            img_show = draw_points(img_show.numpy(), ref_pts)
    ...
    def detect(self, prob_threshold=0.6, area_threshold=100):
        ...
        track_instances = None # object query and track query
        res = {}
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            if args.visualization:
                # ref_pts = res['ref_pts'][0]
                ref_pts = None
                # for visual
                cur_vis_img_path = os.path.join(self.save_path, '{:06d}.jpg'.format(i))
                gt_boxes = None
                self.visualize_img_with_bbox(cur_vis_img_path, ori_img, dt_instances, ref_pts=ref_pts, gt_boxes=gt_boxes)
...
if __name__ == '__main__':
    ...
    for seq_num in seq_nums:
        ...
        # seq_num = ['0005', 'right-cars-in-silver.json']
        # seq_num = ['0013', 'men-in-the-left.json']
        # seq_num = ['0011', 'white-cars-in-the-right.json']
        # seq_num = ['0008', 'moving-cars.json']
